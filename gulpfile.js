...
var src = {
        js: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/mobile/js') + '/**/*.js'
        ],
        css: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/mobile/css') + '/**/*.less'
        ],
        img: [
            '../php_xingguang/static_guojiang_tv/src/mobile/img/**'
        ],
        base: '../php_xingguang/static_guojiang_tv/src/mobile/'
    },
    dest = {
        jscss: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/mobile/js') + '/**/*.js',
            fs.realpathSync('../php_xingguang/static_guojiang_tv/mobile/css') + '/**/*.css'
        ]
    },
    pc_src = {
        js: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/pc/js') + '/**/*.js'
        ],
        less: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/pc/css') + '/**/*.less'
        ],
        css: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/pc/css') + '/**/*.css'
        ],
        cssall: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/src/pc/css') + '/**'
        ],
        img: [
            '../php_xingguang/static_guojiang_tv/src/pc/img/**'
        ],
        base: '../php_xingguang/static_guojiang_tv/src/pc/'
    },
    pc_dest = {
        jscss: [
            fs.realpathSync('../php_xingguang/static_guojiang_tv/pc/v3/js') + '/**/*.js',
            fs.realpathSync('../php_xingguang/static_guojiang_tv/pc/v3/css') + '/**/*.css'
        ]
    },
    output = '../php_xingguang/static_guojiang_tv/mobile',
    pc_output = '../php_xingguang/static_guojiang_tv/pc/v3';
...
gulp.task('rev', function() {
    ...
    gulp.src(['./rev/mobile/img/*.json', '../php_xingguang/static_guojiang_tv/mobile/css/*' ])  
        .pipe( revCollector({
            replaceReved: true
        }) )   
        .pipe(gulp.dest( fs.realpathSync('../php_xingguang/static_guojiang_tv/mobile/css/') ));

    gulp.src(['./rev/mobile/img/*.json', '../php_xingguang/static_guojiang_tv/mobile/js/*' ])  
        .pipe( revCollector({
            replaceReved: true
        }) )   
        .pipe(gulp.dest( fs.realpathSync('../php_xingguang/static_guojiang_tv/mobile/js/') ));     
});
...
gulp.task('pc_rev', function() {
    ...
    gulp.src(['./rev/pc/img/*.json', '../php_xingguang/static_guojiang_tv/pc/v3/css/*' ])  
        .pipe( revCollector({
            replaceReved: true
        }) )   
        .pipe(gulp.dest( fs.realpathSync('../php_xingguang/static_guojiang_tv/pc/v3/css/') ));

    gulp.src(['./rev/pc/img/*.json', '../php_xingguang/static_guojiang_tv/pc/v3/js/*' ])  
        .pipe( revCollector({
            replaceReved: true
        }) )   
        .pipe(gulp.dest( fs.realpathSync('../php_xingguang/static_guojiang_tv/pc/v3/js/') ));     
});
    ...
    fs.readFile(file, function(err, obj){
        ...
        for(var i = 0; i< obj.length; i++){
            ...
            if(srcFile.indexOf('static_guojiang_tv') != -1){
                ...
                gulp.src(srcFile, {base: '../php_xingguang/static_guojiang_tv'})    
...
gulp.task('pc_old', function(){
    ...
    return gulp.src('../php_xingguang/static_guojiang_tv/pc/css/**/*.less')
        .pipe(less()).on('error', errorHandler)
        .pipe(plumber())
        .pipe( gulpif(isOldRelease, cleanCss({compatibility: 'ie7'})) )
        .pipe(debug({title: 'css:'}))
        .pipe(gulp.dest('../php_xingguang/static_guojiang_tv/pc/css'));
});
gulp.task('pc_old_dev', function(){
    isOldRelease = false;
    gulp.start('pc_old');

    var less_watcher = gulp.watch('../php_xingguang/static_guojiang_tv/pc/css/**/*.less', ['pc_old']);
    less_watcher.on('change', function(event) {
      console.log('File ' + event.path + ' was ' + event.type + ', running tasks...');
    })
});
